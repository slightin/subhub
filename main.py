...
re.sub(r"( \-.*steam.*\,)🎯 全球直连",r"\1🔰 节点选择", aritext)
re.sub(r"🇦🇨(.*美国)",r"🇺🇸\1", test)
re.sub(r"🇦🇨(.*香港)",r"🇭🇰\1", test)
re.sub(r"🇦🇨(.*日本)",r"🇯🇵\1", test)
re.sub(r"🇦🇨(.*台湾)",r"🇹🇼\1", test)
re.sub(r"🇦🇨(.*英国)",r"🇬🇧\1", test)
re.sub(r"🇦🇨(.*俄罗斯)",r"🇷🇺\1", test)
f=open('Ari+','w')
f.write(aritext)
f.close()